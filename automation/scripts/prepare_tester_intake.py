...
def build_packet(task: dict[str, Any], latest_handoff: str | None, execution_target: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Suggested Test Commands"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare one tester intake packet from the captain task registry.")
    parser.add_argument("--registry-path", required=True)
    parser.add_argument("--handoffs-dir", required=True)
    parser.add_argument("--execution-target-path", required=True)
    parser.add_argument("--owner", default="aic-tester")
    parser.add_argument("--state", action="append", default=[])
    parser.add_argument("--packet-dir", default="verification-intake")
    ...
    if args.format == "md":
        lines = ["# tester_intake", "", f"- count: {len(items)}"]
        for item in items:
            lines.extend(
                [
                    "",
