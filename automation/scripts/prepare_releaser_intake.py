...
def build_packet(task: dict[str, Any], latest_handoff: str | None, execution_target: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- release_command: {execution_target['release_command'] or 'none'}",
        f"- rollback_command: {execution_target['rollback_command'] or 'none'}",
        "",
        "## Observe Checks",
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare one releaser intake packet from the captain task registry.")
    parser.add_argument("--registry-path", required=True)
    parser.add_argument("--handoffs-dir", required=True)
    parser.add_argument("--execution-target-path", required=True)
    parser.add_argument("--owner", default="aic-releaser")
    parser.add_argument("--state", action="append", default=[])
    parser.add_argument("--packet-dir", default="release-intake")
        ...
        items.append(
            {
                ...
                "release_mode": execution_target["release_mode"],
            }
        )

    result = {"ok": True, "count": len(items), "items": items}
    if args.format == "md":
        lines = ["# releaser_intake", "", f"- count: {len(items)}"]
        for item in items:
            lines.extend(
                [
                    "",
                    ...
                    f"- release_mode: {item['release_mode']}",
                ]
            )
        print("\n".join(lines) + "\n", end="")
        return 0
