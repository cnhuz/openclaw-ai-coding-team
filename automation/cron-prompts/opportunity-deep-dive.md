...
4. 读取该机会已有证据链接，对每个关键 URL 先运行：
   - `python3 scripts/plan_tool_route.py --site-profiles data/research/site_profiles.json --tool-profiles data/research/tool_profiles.json --inventory data/skills/inventory.json --url ... --target-kind article|post|timeline --format md`
5. 按 route 依次尝试 `web.fetch` / `web.search` / `browser`，每次都记录：
   - `python3 scripts/record_tool_attempt.py --attempts-root data/research/tool_attempts --site-id ... --site-label ... --domain ... --tool-id ... --stage opportunity-deep-dive --topic-id ... --url ... --target-kind article|post|timeline --outcome success|failure|partial|skipped --failure-kind ... --quality none|weak|medium|strong --note ...`
6. 如果同一站点在多个周期里反复失败，并且明显缺一个更合适的 skill：
   - 先看 `openclaw skills list --json`
   - 再用 `npx --yes clawhub search "..."` 和 `npx --yes clawhub inspect ... --json`
   - 记录为 skill candidate，而不是在日志里空喊“以后再装”
7. 如果发现新的高价值证据：
...
9. 将本轮结果写入 `data/exec-logs/opportunity-deep-dive/`

规则：

- 本 job 只做深挖和固化 Opportunity Card，不直接创建正式交付任务
- 证据不足时宁可保持 `candidate`，不要过早晋升
- 访问失败时必须切下一种工具，而不是在同一种工具上空转
