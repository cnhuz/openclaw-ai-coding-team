...
- `aic-builder` 作为产品工程，默认调度 `Codex` / `Claude Code` 等编码引擎
...
| Agent ID | 角色 | 核心职责 |
|----------|------|----------|
| `aic-captain` | 经营总控 | 唯一入口、优先级裁决、唯一对外汇报 |
| `aic-planner` | 产品策划 | 需求收敛、任务拆解、验收标准 |
| `aic-reviewer` | 方案评审 | 审查方案与产出，卡边界和质量 |
| `aic-dispatcher` | 运营调度 | 决定派发对象、推进执行闭环 |
| `aic-researcher` | 市场研究 | 研究需求、方向、竞品、用户与技术机会 |
| `aic-builder` | 产品工程 | 编排编码引擎完成实现、审查 diff、推动验证 |
| `aic-tester` | 质量验证 | 测试、回归、评审、验收 |
| `aic-releaser` | 发布运营 | 部署、上线、回滚、发布后观察 |
| `aic-curator` | 知识治理 | 记忆整理、知识分类、术语表、项目档案 |
| `aic-reflector` | 复盘改进 | 日/周复盘、识别低效、推动规则更新 |
